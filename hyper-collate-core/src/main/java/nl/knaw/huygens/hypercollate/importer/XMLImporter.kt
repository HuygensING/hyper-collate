...
class XMLImporter {
    ...
    constructor(
            tokenizer: Function<String, Stream<String>>, normalizer: Function<String, String>) {
    ...
    fun importXML(sigil: String?, input: InputStream?): VariantWitnessGraph {
    ...
    private fun handleEndDocument(event: XMLEvent, context: Context) {
    ...
    private fun handleNotationDeclaration(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: NotationDeclaration")
    }

    private fun handleEntityDeclaration(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: EntityDeclaration")
    }

    private fun handleNameSpace(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: NameSpace")
    }

    private fun handleCData(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: CData")
    }

    private fun handleDTD(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: DTD")
    }

    private fun handleAttribute(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: Attribute")
    }

    private fun handleEntityReference(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: EntityReference")
    }

    private fun handleSpace(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: Space")
    }

    private fun handleComment(event: XMLEvent, context: Context) {}
    private fun handleProcessingInstruction(event: XMLEvent, context: Context) {
        throw RuntimeException("unexpected event: ProcessingInstruction")
    }

    private class Context internal constructor(private val graph: VariantWitnessGraph, // tokenvertex after the </del> yet
                                               private val normalizer: Function<String, String>, witness: SimpleWitness) {
        ...
        fun openMarkup(markup: Markup) {
            ...
            if (!inAppStack.peek() && isVariationStartingMarkup(markup)) { // del
                ...
                rdg = markup.getAttributeMap()["varSeq"]
                if (rdg == null) {
                    rdg = rdgCounter.getAndIncrement().toString()
                }
                if (isLitRdg(markup)) {
        ...
        private fun isLitRdg(markup: Markup): Boolean {
            return "lit" == markup.getAttributeValue("type").orElse("")
        }
        ...
        fun addNewToken(content: String) {
            ...
            token.normalizedContent = normalizer.apply(content)
            val tokenVertex = SimpleTokenVertex(token)
            val branchPath = branchIds.sortedDescending()
