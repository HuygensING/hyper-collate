...
class Match(vararg matchingTokenVertices: TokenVertex) {
    ...
    fun getTokenVertexForWitness(siglum: String): TokenVertex? =
            tokenVertexMap[siglum]

    fun setRank(siglum: String, rank: Int): Match {
        rankingMap[siglum] = rank
        return this
    }

    fun getRankForWitness(siglum: String): Int? =
            rankingMap[siglum]
...
//                    .minOrNull() ?: error("no minimum found") // 2020-09-04: results in java.lang.NoSuchMethodError: 'java.lang.Comparable kotlin.collections.CollectionsKt.minOrNull(java.lang.Iterable)'
    ...
    override fun toString(): String {
        ...
        tokenVertexMap.forEach { (siglum: String, vertex: TokenVertex) ->
            val vString = StringBuilder()
            if (vertex is SimpleTokenVertex) {
                vString.append(siglum).append(vertex.indexNumber)
                // vString.append(siglum)
                // .append("[")
                // .append(sv.getIndexNumber())
                // .append(",r")
                // .append(rankingMap.get(siglum))
                // .append("]:'")
                // .append(sv.getContent().replace("\n", "\\n"))
                // .append("'");
            } else {
                vString.append(siglum).append(":").append(vertex.javaClass.simpleName)
            }
    ...
    val witnessSigla: List<String>
        get() = ArrayList(tokenVertexMap.keys)

    fun addTokenVertex(tokenVertex: SimpleTokenVertex): Match {
        tokenVertexMap[tokenVertex.siglum] = tokenVertex
        return this
    }

    fun hasWitness(siglum: String): Boolean =
            tokenVertexMap.containsKey(siglum)

    init {
        for (mtv in matchingTokenVertices) {
            val siglum = mtv.siglum
            tokenVertexMap[siglum] = mtv
        }
    }
}
